...
SET EXPER=0
::Dynamically guarantee a specific racer in lane 1 - answer "c" or "m"
SET MANUAL=0
...
IF %MANUAL%==1 SET /P MANUALHORSE=Choose a racer: 

ECHO Racers:

::Set up horse pool
::This is done outside of :SELECTHORSE to avoid duplication of work - it takes a moment to concatenate the strings
::TODO: Pass the pool back out so that it can be modified to promote specific duos if one is selected?
SET EQUUS=AMR_SUN_DRM_NUG_LAK_TRX_OLV_APH_IVW_PPB_MWT_CHK_MBL_JAD_DAZ_CXA_MNG_CHY_PnW_TWI_VIN_OCT_CLD_RSH_TWK_WoU
SET MOONLIGHTERS=AMR_NUG_LAK_TRX_OLV_IVW_PPB_MWT_CHK_CXA_MNG_TWI_VIN_OCT_CLD_TWK
SET SUNLIGHTSAVIORS=SUN_MBL_JAD_DAZ_TWI_APH_PnW
SET SALVATIONREACHERS=DRM_RSH
SET UMBRUM=LAK_APH_NUG_AMR_WoU_STL_LUN
SET BASKET=AMR_SUN_DRM_PEP_STP
SET HRT=OOB_SUP_COM_FUZ_PEP_PEP
SET UMA=URA_GOL_STP
...
IF %RANDOM% LEQ 8000 SET HORSEPOOL=%BASKET%_%BASKET%_%BASKET%_%BASKET%
...
IF %MANUAL%==1 CALL :SELECTHORSE %MANUALHORSE% HORSE1ARR HORSE1NAM HORSE1CHAR HORSE1FULLNAME
...
SET FIELDS=GGGGGGGGGDDDDDDDDDDDDDDDDDDDDCCCCWWWVH
...
SET PRIZES=CCCCCCCCCAAAAAAABBBBPPRFFF$$$$MMMSSSiicccGGHHXmmyahhf99bwrppg
...
IF %PRIZECHAR% EQU r SET PRIZENAME=rocks
IF %PRIZECHAR% EQU p SET PRIZENAME=mystery potion
IF %PRIZECHAR% EQU g SET PRIZENAME=gun
...
IF %INPUT%==n EXIT /B 0
IF %INPUT%==N EXIT /B 0
IF %INPUT%==y CLS & SET EXPER=0 & SET MANUAL=0 & GOTO RACE
IF %INPUT%==Y CLS & SET EXPER=0 & SET MANUAL=0 & GOTO RACE
IF %INPUT%==m CLS & SET MANUAL=1 & GOTO RACE
IF %INPUT%==M CLS & SET MANUAL=1 & GOTO RACE
IF %INPUT%==c CLS & SET MANUAL=1 & GOTO RACE
IF %INPUT%==C CLS & SET MANUAL=1 & GOTO RACE
...
SET HORSEARR=??? ? ??????
IF %HORSENAM%==AMR SET HORSEARR=%HORSENAM% A Amrita & IF %RANDOM% LEQ 18000 SET HORSEARR=%HORSENAM% A Amrita of Canterhorn
IF %HORSENAM%==AMR IF %RANDOM% LEQ 12000 SET HORSEARR=TEC A The Entire Circus
IF %HORSENAM%==AMR IF %RANDOM% LEQ 12000 SET HORSEARR=%HORSENAM% A Amrita, The Entire Circus
IF %HORSENAM%==AMR IF %RANDOM% LEQ 6000 SET HORSEARR=WIT A Amrita AS: The Witness of Evyrfree
IF %HORSENAM%==SUN SET HORSEARR=%HORSENAM% S Sunny Field & IF %RANDOM% LEQ 9000 SET HORSEARR=%HORSENAM% S Captain Sunny Field
IF %HORSENAM%==DRM SET HORSEARR=%HORSENAM% d Orange Dream
IF %HORSENAM%==STP SET HORSEARR=%HORSENAM% w Striped Sweater & IF %RANDOM% LEQ 12000 SET HORSEARR=%HORSENAM% w Striped Christmas Sweater
...
IF %HORSENAM%==TRX SET HORSEARR=%HORSENAM% T Trixie Lulamoon & IF %RANDOM% LEQ 12000 SET HORSEARR=%HORSENAM% T Princess Trixie
...
IF %HORSENAM%==STL SET HORSEARR=%HORSENAM% S A Steel Trap's Likeness
...
IF %HORSENAM%==TWK SET HORSEARR=%HORSENAM% t Madame Twinkletoes
IF %HORSENAM%==MBL SET HORSEARR=%HORSENAM% l Moonbelle Dream & IF %RANDOM% LEQ 12000 SET HORSEARR=%HORSENAM% l Princess Moonbelle
IF %HORSENAM%==JAD SET HORSEARR=%HORSENAM% J Jadehoof
IF %HORSENAM%==DAZ SET HORSEARR=%HORSENAM% Z Dazzleflash
IF %HORSENAM%==CXA SET HORSEARR=%HORSENAM% X Coxa & IF %RANDOM% LEQ 6000 SET HORSEARR=ROS R 'Emerald Rose'
IF %HORSENAM%==MNG SET HORSEARR=%HORSENAM% $ Morning Star & IF %RANDOM% LEQ 12000 SET HORSEARR=%HORSENAM% $ Captain Morning Star, Arisen In Flame
IF %HORSENAM%==LUN SET HORSEARR=%HORSENAM% L Princess Luna
...
IF %HORSENAM%==SUS SET HORSEARR=%HORSENAM% g Susie Goatster
IF %HORSENAM%==RAL SET HORSEARR=%HORSENAM% r Horse Ralsei
IF %HORSENAM%==KRS SET HORSEARR=%HORSENAM% k Kris Dreemutt
::Miscellaneous potential crossovers
IF %HORSENAM%==JOJ SET HORSEARR=%HORSENAM% * Silver Bullet Joestar
IF %HORSENAM%==YSI SET HORSEARR=%HORSENAM% Y Yoshi
